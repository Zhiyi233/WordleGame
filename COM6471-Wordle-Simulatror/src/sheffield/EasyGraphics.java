...
// Fix for bug in java.awt added to drawRectangle by SDN 2/1/15
...
public class EasyGraphics extends JFrame {
	...
	*/
	...
	/**
	* Wait before changing the screen.  Allows a maximum of 20 seconds
	* @param s The number of seconds to wait which must be positive
	*/
	public void waitSeconds(int s) {
		if  (  s > 0 )
			try{
				Thread.sleep(Math.min(s,20)*1000);
			}
			catch ( InterruptedException e ) {
				System.out.println("Something very odd has happened");
				System.exit(0);  //rarely happens
			}
